...
public class Calculator {
	...
	private void calculate(String cmdName) {
		...
		if ((currentOp instanceof app.BinaryOperation) && (cmd == null)) {
			cmd = cmdName;
			setClearText(true);
		} else {
			Operation savedOp = Operations.INSTANCE.getOperation(cmd);
			if (savedOp instanceof app.BinaryOperation) {
				app.BinaryOperation bop = (app.BinaryOperation) savedOp;
				newValue = bop.perform(value, curValue);
			} else if (currentOp instanceof app.UnaryOperation) {
				app.UnaryOperation uop = (app.UnaryOperation) currentOp;
			...
			if (currentOp instanceof calcOps.Equals) {
				cmd = null;
			} else if (currentOp instanceof app.BinaryOperation) {
				cmd = cmdName;
			} else {
				cmd = null;
			}
